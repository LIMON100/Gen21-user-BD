          ...
          children: [
            ...
            Obx(() {
              return AnimatedContainer(
                ...
                child: Obx(() {
                  return Column(
                    children: [Text("Requested Service on".tr).paddingSymmetric(vertical: 20), Text('${DateFormat.yMMMMEEEEd(Get.locale.toString()).format(controller.booking.value.bookingAt)}', style: Get.textTheme.headline5), Text('${DateFormat('HH:mm', Get.locale.toString()).format(controller.booking.value.bookingAt)}', style: Get.textTheme.headline3), SizedBox(height: 20)],
                  );
                }),
              );
            })
    ...
    return Container(
      ...
      child: Obx(() {
        return BlockButtonWidget(
          ...
          //         print("sjnjdsn2");
          //       },
          onPressed: (!(Get.find<SettingsService>().address.value?.isUnknown() ?? true)) ? () async {
                  await Get.toNamed(Routes.BOOKING_SUMMARY);
                }
              : null,

          // (!(Get.find<SettingsService>().address.value?.isUnknown() ?? false))
          //     ? () async {
          //
          //         await Get.toNamed(Routes.BOOKING_SUMMARY);
          //       }
