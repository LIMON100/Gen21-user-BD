...
class MessageItemWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    printWrapped("sjfasj messageItem ${message.toString()}");
    AuthService _authService = Get.find<AuthService>();
    print("sjfhnjksnfj ${ this.message.users.firstWhere((element) => element.id != _authService.user.value.id, orElse: () => User.fromJson({})).name != ""? this.message.users.firstWhere((element) => element.id != _authService.user.value.id, orElse: () => User.fromJson({})).name : "User" }");
                        ...
                        Expanded(
                          child: Text(
                           "${ this.message.users.firstWhere((element) => element.id != _authService.user.value.id, orElse: () => User.fromJson({})).name != ""? this.message.users.firstWhere((element) => element.id != _authService.user.value.id, orElse: () => User.fromJson({})).name : "User" }",
                            ...
                            softWrap: false,
                            style: Get.textTheme.bodyText1.merge(TextStyle(fontWeight: this.message.readByUsers.contains(_authService.user.value.id) ? FontWeight.w400 : FontWeight.w800)),
                          ),
                        ),
                        Text(
